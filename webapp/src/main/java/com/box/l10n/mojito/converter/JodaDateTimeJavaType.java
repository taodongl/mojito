...
public class JodaDateTimeJavaType extends AbstractClassJavaType<DateTime> {
  ...
  public <X> X unwrap(DateTime value, Class<X> type, WrapperOptions options) {
    ...
    return ZonedDateTimeJavaType.INSTANCE.unwrap(
        value.toGregorianCalendar().toZonedDateTime(), type, options);
  }
  ...
  public JdbcType getRecommendedJdbcType(JdbcTypeIndicators indicators) {
    return indicators.getTypeConfiguration().getJdbcTypeRegistry().getDescriptor(Types.TIMESTAMP);
  }
